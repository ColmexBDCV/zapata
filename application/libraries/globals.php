...
function public_nav_items(array $navArray = null, $maxDepth = 0)
{
    if (!$navArray) {
        $navArray = array(
            array(
                //'label' =>__('Browse All'),
                'label' =>__('Listado Completo'),
                'uri' => url('items/browse'),
            ));
            if (total_records('Tag')) {
                $navArray[] = array(
                    //'label' => __('Browse by Tag'),
                    'label' => __('por Etiquetas'),
                    'uri' => url('items/tags')
                );
            }
            $navArray[] = array(
                'label' => __('Búsqueda Avanzada'),
...
function get_pathconf()
{
    $path = Zend_Registry::get('pathconf');
    return $path;
}

function get_iprestrict()
{
    $iprestrict = Zend_Registry::get('ip_restrict');
    return $iprestrict;
}
...
function restrict_item(){
    $restrict = false;
    if(get_iprestrict() == get_client_ip()){
        $restrict = true;
    }
    return $restrict;
}